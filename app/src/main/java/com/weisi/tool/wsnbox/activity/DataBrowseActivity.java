...
public class DataBrowseActivity
    ...
    //private CommonWarnProcessor mWarnProcessor;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SENSOR_VALUE_UPDATE: {
                    ...
                    if (!isRealTime) {
                        if (sensor.interpretAddResult(measurementValuePosition, isRealTime)
    ...
    private void initializeView() {
        ...
        //BaseSensorAdapterDelegate.setWarnProcessor(mWarnProcessor);
...
//    }
            ...
            case R.id.tv_filter:
                if (mFilterDialog == null) {
                    ...
                    mFilterDialog.addFilterType(
                            ...
                            SensorTypeFilter.getSensorTypeNames(),
    ...
    private boolean[] getSensorTypeFilterStates() {
        ...
        boolean[] result = new boolean[SensorTypeFilter.getSensorTypeNames().length];
    ...
    private class ImportSensorsWithEarliestValueTask
        ...
        public void onMeasurementDataReceived(long measurementValueId, long timestamp, double rawValue) {
            int address = Sensor.Measurement.ID.getAddress(measurementValueId);
    ...
    private class ImportSensorConfigurationsTask extends AsyncTask<Long, SensorManager.ValueContainerConfigurationProvider, SensorManager.ValueContainerConfigurationProvider> {

        @Override
        protected SensorManager.ValueContainerConfigurationProvider doInBackground(Long... paras) {
            if (paras == null || paras.length == 0) {
                return null;
            }
            SensorManager.ValueContainerConfigurationProvider provider = SensorDatabase.importValueContainerConfigurationProvider(paras[0]);
            SensorManager.setValueContainerConfigurationProvider(provider, true);
            return provider;
        }

        @Override
        protected void onPostExecute(SensorManager.ValueContainerConfigurationProvider provider) {
            if (provider != null) {
