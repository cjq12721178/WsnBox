...
//    }
    ...
    private static void importMeasurementConfigurationsWithSingleRangeWarner(
        ...
        try {
            ...
            builder.append("SELECT ")
                    ...
                    .append(',').append(COLUMN_CUSTOM_UNIT)
                    .append(',').append(COLUMN_DECIMALS)
                    .append(",m.").append(COLUMN_TYPE)
                    .append(',').append(COLUMN_CORRECTOR)
            ...
            if (cursor != null) {
                ...
                int customNameIndex = cursor.getColumnIndex(COLUMN_CUSTOM_NAME);
                int customUnitIndex = cursor.getColumnIndex(COLUMN_CUSTOM_UNIT);
                int decimalsIndex = cursor.getColumnIndex(COLUMN_DECIMALS);
                ...
                int correctorIndex = cursor.getColumnIndex(COLUMN_CORRECTOR);
                while (cursor.moveToNext()) {
                    importMeasurementConfiguration(configurationMap,
                            cursor, measurementIdIndex, customNameIndex,
                            customUnitIndex, decimalsIndex, typeIndex,
                            measurementConfigIdIndex, correctorIndex,
                            new CommonSingleRangeWarner(
                                    cursor.getDouble(highLimitIndex),
                                    cursor.getDouble(lowLimitIndex)));
                }
            }
        } catch (Exception e) {
            ExceptionLog.record(e);
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    importMeasurementConfiguration(configurationMap,
                            ...
                            customUnitIndex, decimalsIndex, typeIndex,
                            measurementConfigIdIndex, correctorIndex,
                            new CommonSwitchWarner(cursor.getDouble(abnormalValueIndex)));
                }
            }
        } catch (Exception e) {
            ExceptionLog.record(e);
        ...
        try {
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    importMeasurementConfiguration(configurationMap,
                            cursor, measurementIdIndex, customNameIndex,
                            customUnitIndex, decimalsIndex,
                            typeIndex, measurementConfigIdIndex,
                            correctorIndex, null);
                }
            }
        } catch (Exception e) {
    ...
    private static void importMeasurementConfiguration(
            ...
            int customNameIndex, int customUnitIndex, int decimalsIndex,
            int typeIndex, int measurementConfigIdIndex,
            int correctorIndex,
            Warner<DisplayMeasurement.Value> warner) {
        ID id = new ID(cursor.getLong(measurementIdIndex));
        DisplayMeasurementConfiguration configuration = buildMeasurementConfiguration(cursor,
                customNameIndex, customUnitIndex, decimalsIndex,
                typeIndex, measurementConfigIdIndex,
                correctorIndex, warner);
        configurationMap.put(id, configuration);
    }

    @NonNull
    private static DisplayMeasurementConfiguration buildMeasurementConfiguration(
            Cursor cursor, int customNameIndex,
            int customUnitIndex, int decimalsIndex,
            int typeIndex, int measurementConfigIdIndex,
            int correctorIndex,
            Warner<DisplayMeasurement.Value> warner) {
        return buildMeasurementConfiguration(cursor, customNameIndex,
                customUnitIndex, decimalsIndex, typeIndex,
                cursor.getLong(measurementConfigIdIndex),
                correctorIndex, warner);
    }
    ...
    private static DisplayMeasurementConfiguration buildMeasurementConfiguration(
            Cursor cursor, int customNameIndex,
            int customUnitIndex, int decimalsIndex,
            int typeIndex, long measurementConfigId,
            int correctorIndex,
            Warner<DisplayMeasurement.Value> warner) {
        return buildMeasurementConfiguration(measurementConfigId,
                cursor.getInt(typeIndex),
                cursor.getString(customNameIndex),
                cursor.getString(customUnitIndex),
                cursor.isNull(decimalsIndex) ? -1 : cursor.getInt(decimalsIndex),
                cursor.getBlob(correctorIndex),
                warner);
    }

    @NonNull
    private static DisplayMeasurementConfiguration buildMeasurementConfiguration(
            long measurementConfigId, int type,
            String customName, String customUnit, int decimals,
            byte[] correctorData, Warner<DisplayMeasurement.Value> warner) {
        DisplayMeasurementConfiguration configuration = buildDisplayMeasurementConfigurationByType(type, measurementConfigId);
        String notNullCustomName = TextUtils.isEmpty(customName) ? "" : customName;
        String notNullCustomUnit = TextUtils.isEmpty(customUnit) ? "" : customUnit;
        if (!notNullCustomName.isEmpty() || !notNullCustomUnit.isEmpty()) {
            configuration.setDecorator(new CommonMeasurementDecorator(notNullCustomName, notNullCustomUnit, decimals));
        }
        if (correctorData != null) {
            configuration.setCorrector(CorrectorUtil.buildCorrector(correctorData));
        }
    ...
    public static SensorConfiguration importSensorConfiguration(long sensorConfigId) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                measurementConfiguration = buildMeasurementConfiguration(cursor,
                        customNameIndex, customUnitIndex, decimalsIndex,
                        typeIndex, measurementConfigId, correctorIndex,
                        warner);
                sensorConfiguration.setMeasureConfiguration(measurementId,
                        measurementConfigId, measurementConfiguration);
            }
            return sensorConfiguration;
        } catch (Exception e) {
            ExceptionLog.record(e);
    ...
    private static Warner<DisplayMeasurement.Value> importMeasurementWarner(long measurementConfigId, StringBuilder builder) {
        ...
        try {
            ...
            cursor = database.rawQuery(builder.toString(), null);
            if (cursor != null && cursor.moveToNext()) {
                return new CommonSingleRangeWarner(
                        cursor.getDouble(cursor.getColumnIndex(COLUMN_HIGH_LIMIT)),
                        cursor.getDouble(cursor.getColumnIndex(COLUMN_LOW_LIMIT)));
            }
            if (cursor != null) {
                cursor.close();
            }
            ...
            cursor = database.rawQuery(builder.toString(), null);
            if (cursor != null && cursor.moveToNext()) {
                return new CommonSwitchWarner(cursor.getDouble(cursor.getColumnIndex(COLUMN_ABNORMAL_VALUE)));
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    private static class SQLiteLauncher extends SQLiteOpenHelper {
        ...
        private void createMeasurementConfigurationDataTable(SQLiteDatabase db, StringBuilder builder) {
            ...
            db.execSQL(builder.append("CREATE TABLE ")
                    ...
                    .append(COLUMN_MEASUREMENT_VALUE_ID).append(" BIGINT NOT NULL,")
                    .append(COLUMN_CUSTOM_NAME).append(" VARCHAR(255),")
                    .append(COLUMN_CUSTOM_UNIT).append(" VARCHAR(255),")    //版本VN_CORRECTOR添加
                    .append(COLUMN_DECIMALS).append(" INT,")    //版本VN_CORRECTOR添加
                    .append(COLUMN_TYPE).append(" INT,")   //版本VN_VIRTUAL_CONFIG添加，版本VN_GOOD_TYPE将类型从VARCHAR(255)改为INT
                    .append(COLUMN_CORRECTOR).append(" BLOB,")  //版本VN_CORRECTOR添加
...
//            db.execSQL(builder.toString());
        ...
        private void addColumnCorrectorFromTableMeasurementConfiguration(SQLiteDatabase db, StringBuilder builder) {
            changeTableToTemporaryTable(db, builder, TABLE_MEASUREMENT_CONFIGURATION);
            createMeasurementConfigurationDataTable(db, builder);
            //insertDataFromTemporaryTableToTargetTable(db, builder, TABLE_MEASUREMENT_CONFIGURATION);
            builder.setLength(0);
            builder.append("INSERT INTO ").append(TABLE_MEASUREMENT_CONFIGURATION)
                    ...
                    .append(',').append(COLUMN_SENSOR_CONFIGURATION_ID)
                    .append(',').append(COLUMN_MEASUREMENT_VALUE_ID)
                    .append(',').append(COLUMN_CUSTOM_NAME)
                    .append(',').append(COLUMN_TYPE)
                    .append(") SELECT * FROM ").append(TABLE_MEASUREMENT_CONFIGURATION)
    ...
    private static class ValueContainerConfigurationProviderHandler extends DefaultHandler {
        ...
        private int mMeasurementDecimals;
        ...
        private List<Float> mCorrectedValues;
        private List<Float> mSamplingValues;
        ...
        public void endElement(String uri, String localName, String qName) {
            switch (localName) {
                ...
                case TAG_MEASUREMENT:
                    ...
                    if (!TextUtils.isEmpty(mMeasurementUnit)) {
                        mValues.put(COLUMN_CUSTOM_UNIT, mMeasurementUnit);
                        mMeasurementUnit = null;
                    }
                    if (mMeasurementDecimals != -1) {
                        mValues.put(COLUMN_DECIMALS, mMeasurementDecimals);
                        mMeasurementDecimals = -1;
                    }
                    ...
                    if (mCorrector != null) {
                        mValues.put(COLUMN_CORRECTOR, CorrectorUtil.toByteArray(mCorrector));
                        mCorrector = null;
                    }
                ...
                case TAG_INIT_VALUE:
                    ...
                    break;
                case TAG_INIT_INSTANCE:
                    mInitDistance = Double.parseDouble(mBuilder.toString());
                    break;
                case TAG_UNIT:
                    mMeasurementUnit = mBuilder.toString();
                    break;
                case COLUMN_DECIMALS:
                    mMeasurementDecimals = Integer.parseInt(mBuilder.toString());
                    break;
                case TAG_CORRECTED_VALUE:
                    if (mCorrectedValues == null) {
                        mCorrectedValues = new ArrayList<>();
                    }
                    mCorrectedValues.add(Float.parseFloat(mBuilder.toString()));
                    break;
                case TAG_SAMPLING_VALUE:
                    if (mSamplingValues == null) {
                        mSamplingValues = new ArrayList<>();
                    }
                    mSamplingValues.add(Float.parseFloat(mBuilder.toString()));
                    break;
                case COLUMN_CORRECTOR:
                    if (mCorrectedValues != null
                            && mSamplingValues != null
                            && mCorrectedValues.size() == mSamplingValues.size()) {
                        mCorrector = new LinearFittingCorrector(mCorrectedValues,
                                mSamplingValues,
                                "kN", "");
                    }
                    break;
